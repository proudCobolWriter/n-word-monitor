...
const { Client, GatewayIntentBits, Routes, EmbedBuilder, ActivityType, ActionRowBuilder, ButtonBuilder, ButtonStyle } = require("discord.js");
...
client.on("messageCreate", (msg) => {
    ...
    if (checkMessage(lowercaseMessage)) {
        ...
        if (userData) {
            if (msgCooldownData.has(msg.author.id) && msgCooldownData.get(msg.author.id) >= (Date.now() - coooldownMessagesInSec * 1000) ) return;
...
client.on("interactionCreate", (interaction) => {
    if (interaction.isChatInputCommand() && interaction.commandName == "rleaderboard") {
        ...
        const collector = interaction.channel.createMessageComponentCollector({ time: 60*1000 });

        interaction.reply({ embeds: [createEmbed(1, getFields(1))], components: [createActionRow("button1")] }).catch(err => console.log("Couldn't send embed on command 1 : ", err)).finally(() => {
            ...
            try {
                collector.on('collect', async i => {
                    ...
                    if (getFields(updatedPageValue + 1).length == 0 && buttonToGreyOut != "button1") buttonToGreyOut = "button2";

                    await i.update({ embeds: [createEmbed(updatedPageValue, getFields(updatedPageValue))], components: [createActionRow(buttonToGreyOut)] });
                });
            } catch (err) { console.log("Error occured at button collector : ", err); }
